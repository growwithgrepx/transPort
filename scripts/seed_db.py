...
with app.app_context():
    ...
    print('Sample data inserted successfully.')</old_str>
<new_str>import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
try:
...
with app.app_context():
    ...
    # Create users with Flask-Security's user_datastore for proper password hashing
    fleet_manager = user_datastore.create_user(
        username='fleetmanager',
        email='fleetmanager@example.com',
        password='manager123',
    ...
    # Print created users for verification (password is now properly hashed)
    for user in [fleet_manager, system_admin, fleet_employee, accountant, customer_service]:
        print(f"User created: {user.username} ({user.email}) - Password properly hashed")
    ...
    print('Sample data inserted successfully with proper password hashing.')</new_str>
